...
<template>
  <nav class="absolute">
    <div class="relative" :style="{top: navPosY + 'px', width: context.imageContainerWidth + 'px'}">
      <button class="absolute left-2 bg-white rounded-full" v-if="showPrev" @click="onPrevClick($event)">
        <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"
             stroke-width="2">
          <path stroke-linecap="round" stroke-linejoin="round" d="M15 19l-7-7 7-7"/>
        </svg>
      </button>
      <button class="absolute right-2 bg-white rounded-full" v-if="showNext" @click="onNextClick($event)">
        <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"
             stroke-width="2">
